...
The gate table is locked in the private evaluation harness (`SHIP_GATES`).
A regression test in that harness asserts the table covers the five
spec-named gates exactly.
...
1. **Fixture corpus.** ≥10 labelled morning-briefing sessions per
   archetype under the private evaluation fixture corpus. Format:
